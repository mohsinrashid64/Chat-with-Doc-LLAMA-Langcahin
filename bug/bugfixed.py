...
class LlamaCppEmbeddings(BaseModel, Embeddings):
    ...
    def embed_query(self, text: str) -> List[float]:
        ...
        return [list(map(float, e)) for e in embedding]
    ...
    #     """
    #     embedding = self.client.embed(text)
    #     # Handle both cases where embedding might be a nested list or a flat list
    #     if all(isinstance(i, list) for i in embedding):
    #         # Flatten the list if it's nested
    #         return [float(item) for sublist in embedding for item in sublist]
    #     else:
    #         # Directly convert to float if it's a flat list
    #         return list(map(float, embedding))
